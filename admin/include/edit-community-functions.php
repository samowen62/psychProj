<?php
    ...
    function migrateUser($src_community, $dest_community, $username){
        $dbc = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
        $query = "SELECT * FROM community WHERE name='{$src_community}';";
        $result = mysqli_query($dbc, $query);
        while ($row = mysqli_fetch_array($result)){
            $src_id = $row['id'];
        }
        $query = "SELECT * FROM community WHERE name='{$dest_community}';";
        $result = mysqli_query($dbc, $query);
        while ($row = mysqli_fetch_array($result)){
            $dest_id = $row['id'];
        }
        $query = "UPDATE community_has_user 
                    ...
                    WHERE community_id={$src_id} AND username='{$username}';";
        mysqli_query($dbc, $query);
        mysqli_close($dbc);
